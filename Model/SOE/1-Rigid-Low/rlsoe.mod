...
parameters 
        ...
        gamma           $\gamma$                (long_name='Substitution between foreign countries goods')
...
Psis=alpha*(Psia+(1-beta*(1-delt))*M*g);
...
h0s=(Psis*sigma)*(1-net0)/(alpha*omega)+(alp*M*g)/delt+g*M*beta*(1-delt)*(net1-net0+alp*(1-delt)*(1-x_n)/delt);
hLs=-Psis*sigma*net1/(alpha*omega)-alp*M*g*(1-delt)*(1-x_n)/delt-g*M*beta*(1-delt)*net1;
hFs=-g*M*beta*(1-delt)*((alp/delt)-net0);
...
y = a + delt*xhat + (1-delt)*(1-x_n)*n(-1);
...
stoch_simul(TeX,order=1,irf=40,irf_plot_threshold=0);%u pih pi;
